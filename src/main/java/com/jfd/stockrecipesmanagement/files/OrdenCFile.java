 ...
 * @author javie
 */
public class OrdenCFile {
    
    public static void generarOrden(String fecha, String numero, String observ, List<List<String>> detalleProductos ) throws FileNotFoundException, IOException{
        ...
        String pdfPath = "OrdenC"+numero+".pdf";
    ...
    // Código que se ejecuta en cada iteración
        ...
        prodDetTable.addCell(newProductPane(detalleProductos.get(i).get(0)));
        prodDetTable.addCell(newProductPane("    "));
    ...
    private static Cell headerLeftPane() {
        ...
        return new Cell().add("Orden Compra").setBold().setTextAlignment(TextAlignment.CENTER).setFontSize(18f).setVerticalAlignment(VerticalAlignment.MIDDLE);
    }
    ...
    private static Table headerRightPane(String date, String number) throws IOException {
        ...
        nestedTable.addCell(new Cell().add("N° Orden:").setBold().setFontSize(8f).setTextAlignment(TextAlignment.LEFT).setBorder(Border.NO_BORDER).setVerticalAlignment(VerticalAlignment.MIDDLE));
